  ...
  Markit.InteractiveChartApi.prototype._getPrices = function(json) {
    ...
    for(var j=0; j<elements.length; j++) {
      ...
      for (var i = 0, datLen = dates.length; i < datLen; i++) {
        ...
        var pointData = [
          dat,
          elements[j].DataSeries['close'].values[i]
        ];
        cs.push( pointData );
      }
      chartSeries.push(cs);
          ...
          <div className="row">
            <form className="navbar-form navbar-left" role="search">
              <div className="form-group">
                <input type="text" className="form-control" placeholder="股票代码" onChange={this.onChange} value={this.state.text}/>
                  </div>
                <button type="button" className="btn btn-default" onClick={this.addNew}>添加</button>
                </form>
              </div>
            <div className="row">
              {this.state.items.map(
                ...
                <h1>
                  <span className="tag label label-info">
                    <span>{item}</span>
                    <a><i className="remove glyphicon glyphicon-remove-sign glyphicon-white" onClick={this.delete} value={item}></i></a></span>
                </h1> 
              )}
            </div>
          </div>
